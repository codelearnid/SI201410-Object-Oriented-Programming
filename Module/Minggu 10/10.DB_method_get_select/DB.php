...
class DB{
  ...
  public function select($columnName){
    $this->_columnName = $columnName; //tidak menggunakan return karena nilai tidak dikirim
                                      //melainkan dipanggil melalui $this->_columnName
  }

  // Method utama untuk mengambil isi tabel
  public function get($tableName){
    $query = "SELECT {$this->_columnName} FROM {$tableName}";
    $this->_columnName = "*";
    return $this->getQuery($query); //menggunakan return karena nilai query di kirim ke getQuery
  }
}